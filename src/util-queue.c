...
void deinit_queue(struct gen_queue * queue) {
    while (dequeue(queue)) {
	printk(KERN_ERR "Freeing non-empty queue. PROBABLE MEMORY LEAK DETECTED\n");
    }
}
...
struct gen_queue * create_queue(unsigned int max_entries) {
    ...
    if (!tmp_queue) { 
	printk(KERN_ERR "Unable to allocate a queue\n");
	return NULL;
    }
...
int enqueue(struct gen_queue * queue, void * entry) {
    ...
    if (!q_entry) { 
	printk(KERN_ERR "Unable to allocate a queue entry on enqueue\n");
	return -1;
    }
