...
static inline int is_read_loop(struct pisces_early_ringbuf * ring, unsigned int len) {
    if ((ring->read_idx >= ring->write_idx) && (ring->cur_len > 0)) {
...
static inline int is_write_loop(struct pisces_early_ringbuf * ring, unsigned int len) {
    if ((ring->write_idx >= ring->read_idx) && (ring->cur_len < ring->size)) {
	// end is past the end of the buffer
...
int pisces_early_ringbuf_write(struct pisces_early_ringbuf * ring, 
			       u8 * data, u64 write_len) {
...
int pisces_early_ringbuf_read(struct pisces_early_ringbuf * ring, 
			      u8 * data, u64 read_len) {
	...
	memcpy(data + section_len, ring->buf, read_len - section_len);
