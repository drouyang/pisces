...
int buddy_add_pool(struct buddy_memzone * zone, 
    ...
    if (pool_order > zone->max_order) {
        printk(KERN_ERR "Pool order size is larger than max allowable zone size (pool_order=%lu) (max_order=%lu)\n", pool_order, zone->max_order);
        return -1;
    } else if (pool_order < zone->min_order) {
        printk(KERN_ERR "Pool order is smaller than min allowable zone size (pool_order=%lu) (min_order=%lu)\n", pool_order, zone->min_order);
        return -1;
    }
    ...
    if (IS_ERR(mp)) {
        printk(KERN_ERR "Could not allocate mempool\n");
        return -1;
    }
    ...
    if (ret == -1) {
        printk(KERN_ERR "Could not insert mempool into zone\n");
        kfree(mp->tag_bits);
        kfree(mp);
...
static int __buddy_remove_mempool(struct buddy_memzone * zone, 
    ...
    if (pool == NULL) {
        printk(KERN_ERR "Could not find mempool with base address (%p)\n", (void *)base_addr);
        return -1;
    }

    if (!bitmap_empty(pool->tag_bits, pool->num_blocks)) {
        printk(KERN_ERR "Trying to remove an in use memory pool\n");
        return -1;
    }
        ...
        ) 
{
    ...
    if ((pool == NULL) || (order > pool->pool_order)) {
        printk(KERN_ERR "Attempted to free an invalid page address (%p)\n", (void *)addr);
        spin_unlock_irqrestore(&(zone->lock), flags);
        return;
    }
...
void buddy_deinit(struct buddy_memzone * zone, struct proc_dir_entry * parent) {
    unsigned long flags;
        ...
        )
{
    ...
    printk(KERN_DEBUG "Initializing Memory zone with up to %lu bit blocks on Node %d\n", max_order, node_id);
    ...
    if (IS_ERR(zone)) {
        printk(KERN_ERR "Could not allocate memzone\n");
        return NULL;
    }
